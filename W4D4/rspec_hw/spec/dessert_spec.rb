...
describe Dessert do
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("chocolate", "chicken", :chef) }.to raise_error(ArgumentError)
    end
  end

  describe "#add_ingredient" do
    it "adds an ingredient to the ingredients array" do
      brownie.add_ingredient("cheese")
      expect(brownie.ingredients).to eq(["cheese"])
    end
  end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Jimmy the Great Baker")
      expect(brownie.serve).to eq("Chef Jimmy the Great Baker has made 10 chocolates!")
    end
  end
